...
for i=1:1:nrColonies
    ...
    totalAnts = totalAnts + colonies(i).population;
end
sourceProd = [];
for i=1:1:nrSources
   ...
   %for antI=1:1:1 %ONLY TESTING PURPOSES
    ...
    if mod(t,timeInterval) == 0
        ...
        for i=1:1:nrColonies
                ...
                colonyProd(i).intervals(colonyProd(i).interval) = colonyProd(i).intervals(colonyProd(i).interval) / timeInterval;

                colonyPord(i).foodCounterGlobal = colonyProd(i).foodCounterGlobal + colonyProd(i).foodCounter;
                
                s = s + colonyProd(i).foodCounter;
        ...
        globalProd.intervals(end+1) = s/(totalAnts*timeInterval);
        for i=1:1:nrSources
                % add a new entry
